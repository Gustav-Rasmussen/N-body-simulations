...
if betaGammaFunctions:
    ...
    def gammaHQ(r, rho0=1. / (2. * np.pi), x=1.):
        x = rhoHQ(r, rho0, x)
        gamma_HQ = []
        for i in range(len(r)):
            ...
            dlogrho = np.log10(x[i + 1]) - np.log10(x[i - 1])
    ...
    def kappa(r):
        ...
        for i in range(len(r)):
            if i in (0, len(r) - 1):
                kappaArr.append(np.nan)
                continue
            dlogr = np.log10(r[i + 1]) - np.log10(r[i - 1])
            dlogsigmarad2 = np.log10(x[i + 1]) - np.log10(x[i - 1])
            kappaArr.append(dlogsigmarad2 / dlogr)
        return np.array(kappaArr)
