    ...
    for root, dirs, files in os.walk("."):
        for filename in files:
            if ((filename in (".DS_Store", "KPI.log"))
                ...
                continue
            lines = subprocess.check_output(line_count.format(filename),
                                            shell=True)
            PEP8 = subprocess.check_output(PEP8_count.format(filename),
                                           shell=True)
            ...
            file_count += 1
        KPI_list_sorted = sorted(KPI_list, key=lambda KPI: KPI['lines'],
                                 reverse=True)
