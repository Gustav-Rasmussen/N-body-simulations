...
bins_202                               = 0
bins_102                               = 0
larger_fewer_bins                      = 1 # Reduce number of radial bins in analysis code. This makes them larger and they therefore contain more particles.
...
if F.startswith('A_') or F.startswith('B_') or F.startswith('E_'):
    N = 10**6
elif F.startswith('CS4_') or F.startswith('CS5_') or F.startswith('CS6_') or F.startswith('DS1_') or F.startswith('D2_'):
    N = 10**5
...
if F.startswith('A_') or F.startswith('B_') or F.startswith('CS4_') or F.startswith('CS5_') or F.startswith('CS6_') or F.startswith('E_'):
    M = 1. # Total mass equals unity
elif F.startswith('DS1_') or F.startswith('D2_'):
    M = 1./6.
...
binning_arr_lin_log10_unitRmax = 10**((np.arange(nr_binning_bins)/(nr_binning_bins-1))*abs(np.log10(max_binning_R_unitRmax)-np.log10(min_binning_R_unitRmax)) + np.log10(min_binning_R_unitRmax)) # Array, 0.00001-1.
binning_arr_lin_log10          = R_limit*binning_arr_lin_log10_unitRmax # Array, 0-500
for i in range(0, int(nr_binning_bins-2)):                                   # loop over 0-998
    ...
    nr_par_inside_bin_i   = len(posR_par_inside_bin_i)                       # number of particles inside a radial bin
...
if Fig2_Density:
    ...
    plt.plot(x_plot[0:int(nr_binning_bins-2)],np.log10(rho_arr),'-o',ms=2,lw=2,mew=0,c='green',label=r'$\rho$')
    #plt.legend(prop=dict(size=12), numpoints=2, ncol=2,frameon=True,loc=1,handlelength=2.5)

    if Fig2_Densityfit:
        x         = binning_arr_lin_log10
        y_plot    = rho_Hernquist(1./(2*np.pi),1.,x)   
        plt.plot(np.log10(x),np.log10(y_plot),':o',ms=2,lw=2,mew=0,c='black',label=r'$\frac{1}{2\pi r (1+r)^3}$')
        ...
        #while (300 < i < len(rho_arr)-300):    # returns chi2 = 0.00000
        #while (1 < i < len(rho_arr)-1):       # returns chi2 = 0.00000
            
            if isnan(rho_arr[i]):
                print 'nan at index: ', i
            else:
                Chi2 = Chi2 + ((rho_arr[i]-y_plot[i])**2)/(rho_arr[i]*.2)**2
...
if Fig2_Density_r_2:
    ...
    plt.ylabel(r'$ \log \rho $',fontsize=30)
    plt.plot(x_plot[0:int(nr_binning_bins-2)],np.log10(rho_arr),'-o',ms=2,lw=2,mew=0,c='green',label=r'$\rho$')
    #plt.legend(prop=dict(size=18), numpoints=2, ncol=2,frameon=True,loc=1,handlelength=2.5)
...
# 2 plots of the potential as function of radius. 2.nd plot has logarithmic radius.
if Fig3_Potential:
    f   = plt.figure()
    ax1 = plt.subplot(121)
    plt.xlabel('r')
